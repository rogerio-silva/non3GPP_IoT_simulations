...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("sideLength", "Radius of the deployment", sideLength);
  cmd.Parse (argc, argv);

  int appPeriodSeconds = simulationTime;

  // Set up logging
//  LogComponentEnable ("AlohaThroughput", ns3::LOG_LEVEL_ALL);
...
//  Config::SetDefault ("ns3::EndDeviceLorawanMac::DataRate", UintegerValue (5));
  ...
  for (NodeContainer::Iterator j = endDevices.Begin (); j != endDevices.End (); ++j)
    {
      Ptr<MobilityModel> mobED = (*j)->GetObject<MobilityModel> ();
      Vector position = mobED->GetPosition ();
      position.z = 1.2;
      mobED->SetPosition (position);
    }
  ...
  // Make it so that nodes are at a certain height > 0
  allocator->Add (Vector (0.0, 0.0, 30.0));
