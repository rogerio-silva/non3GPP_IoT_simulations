...
/*
 * Copyright (c) 2018 Universidade Federal de Goiás
 *
...
NS_LOG_COMPONENT_DEFINE ("NoAdrComponent");

NS_OBJECT_ENSURE_REGISTERED (NoAdrComponent);

TypeId NoAdrComponent::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NoAdrComponent")
    .SetGroupName ("lorawan")
    .AddConstructor<NoAdrComponent> ()
    ...
    .AddAttribute ("MultipleGwCombiningMethod",
                   ...
                   EnumValue (NoAdrComponent::AVERAGE),
                   MakeEnumAccessor (&NoAdrComponent::tpAveraging),
                   MakeEnumChecker (NoAdrComponent::AVERAGE,
                                    "avg",
                                    NoAdrComponent::MAXIMUM,
                                    "max",
                                    NoAdrComponent::MINIMUM,
                                    "min"))
    .AddAttribute ("MultiplePacketsCombiningMethod",
                   "Whether to average SNRs from multiple packets or to use the maximum",
                   EnumValue (NoAdrComponent::AVERAGE),
                   MakeEnumAccessor (&NoAdrComponent::historyAveraging),
                   MakeEnumChecker (NoAdrComponent::AVERAGE,
                                    "avg",
                                    NoAdrComponent::MAXIMUM,
                                    "max",
    ...
    .AddAttribute ("HistoryRange",
                   ...
                   IntegerValue (4),
                   MakeIntegerAccessor (&NoAdrComponent::histRange),
                   MakeIntegerChecker<int> (0, 100))
    .AddAttribute ("ChangeTransmissionPower",
                   "Whether to toggle the transmission power or not",
                   BooleanValue (true),
                   MakeBooleanAccessor (&NoAdrComponent::m_toggleTxPower),
                   MakeBooleanChecker ())
  ;
  return tid;
}

NoAdrComponent::NoAdrComponent ()
{
}
...
void NoAdrComponent::OnReceivedPacket (Ptr<const Packet> packet,
...
void
NoAdrComponent::BeforeSendingReply (Ptr<EndDeviceStatus> status,
                                  Ptr<NetworkStatus> networkStatus)
{
  ...
  if (fHdr.GetAdr ())
    {
      if (int(status->GetReceivedPacketList ().size ()) < histRange)
        {
          NS_LOG_ERROR ("Not enough packets received by this device (" << status->GetReceivedPacketList ().size () << ") for the algorithm to work (need " << histRange << ")");
        }
      else
        {
          ...
          //ADR Algorithm
          NoAdrImplementation (&newDataRate,
                             &newTxPower,
                             status);
...
void NoAdrComponent::OnFailedReply (Ptr<EndDeviceStatus> status,
                                  Ptr<NetworkStatus> networkStatus)
{
  NS_LOG_FUNCTION (this->GetTypeId () << networkStatus);
}

void NoAdrComponent::NoAdrImplementation (uint8_t *newDataRate,
                                      uint8_t *newTxPower,
                                      Ptr<EndDeviceStatus> status)
{
...
uint8_t NoAdrComponent::SfToDr (uint8_t sf)
{
  switch (sf)
    {
    case 12:
...
double NoAdrComponent::RxPowerToSNR (double transmissionPower)
{
...
double NoAdrComponent::GetMinTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
...
//Get the maximum received power (it considers the values in dB!)
double NoAdrComponent::GetMaxTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
  EndDeviceStatus::GatewayList::iterator it = gwList.begin ();
...
double NoAdrComponent::GetAverageTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
  double sum = 0;
...
double
NoAdrComponent::GetReceivedPower (EndDeviceStatus::GatewayList gwList)
{
  switch (tpAveraging)
    {
    case NoAdrComponent::AVERAGE:
...
double NoAdrComponent::GetMinSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                int histRange)
{
  double m_SNR;
...
double NoAdrComponent::GetMaxSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                int histRange)
{
  double m_SNR;
...
double NoAdrComponent::GetAverageSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                    int histRange)
{
  double sum = 0;
  double m_SNR;
...
int NoAdrComponent::GetTxPowerIndex (int txPower)
{
  if (txPower >= 16)
    {
      return 0;
